...
async def connect_to_snowflake_async():
    """Async wrapper for Snowflake connection"""
    return await asyncio.to_thread(connect_to_snowflake)

async def setup_snowflake_session_async(connection, config):
    """Async wrapper for session setup"""
    return await asyncio.to_thread(setup_snowflake_session, connection, config)

async def execute_snowflake_query_async(cursor, query, params=None):
    """Async wrapper for query execution"""
    return await asyncio.to_thread(execute_snowflake_query, cursor, query, params)
