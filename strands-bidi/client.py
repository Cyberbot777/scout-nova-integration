#!/usr/bin/env python3
"""
Scout Voice Agent Client Launcher
Starts a web service that serves the voice client HTML with pre-configured WebSocket URLs.

Supports two modes:
1. Local development: Direct WebSocket connection (ws://localhost:8080/ws)
2. AgentCore production: Pre-signed WebSocket URL with SigV4 authentication
...
Usage:
    ...
    python client.py --runtime-arn arn:aws:bedrock-agentcore:us-east-1:123456789012:runtime/abc123
"""
import argparse
import os
import sys
...
class ScoutClientHandler(BaseHTTPRequestHandler):
    """HTTP request handler that serves the Scout voice client"""
    ...
    def serve_client_page(self):
        ...
        try:
            ...
            # Inject agent name if provided
            if self.agent_name:
                html_content = html_content.replace(
                    'BidiAgent WebSocket',
                    f'{self.agent_name} Voice Agent'
                )
            ...
            self.send_error(404, "client.html not found")
    ...
    def regenerate_url(self):
        ...
        try:
            ...
            base_url = f"wss://bedrock-agentcore.{self.region}.amazonaws.com/runtimes/{self.runtime_arn}/ws?qualifier={self.qualifier}&voice_id={self.voice_id}"
            ...
            ScoutClientHandler.websocket_url = new_url
...
def main():
    parser = argparse.ArgumentParser(
        description="Start web service for Scout voice client",
...
Examples:
  ...
  # Specify custom port and voice
  python client.py --runtime-arn arn:aws:bedrock-agentcore:us-east-1:123456789012:runtime/RUNTIMEID \\
    --port 8000 --voice-id matthew
...
""",
    ...
    parser.add_argument(
        "--voice-id",
        default="matthew",
        help="Nova Sonic voice ID (default: matthew)",
    )

    parser.add_argument(
        "--agent-name",
        default="Scout",
        help="Agent name to display in UI (default: Scout)",
    )
    ...
    print("=" * 70)
    print(f"🎙️ {args.agent_name} Voice Agent - Client Launcher")
    print("=" * 70)
    ...
    try:
        ...
        if args.runtime_arn:
            base_url = f"wss://bedrock-agentcore.{args.region}.amazonaws.com/runtimes/{args.runtime_arn}/ws?qualifier={args.qualifier}&voice_id={args.voice_id}"

            print(f"📡 Base URL: {base_url[:100]}...")
            print(f"🔑 Runtime ARN: {args.runtime_arn}")
            print(f"🌍 Region: {args.region}")
            print(f"🎙️ Voice ID: {args.voice_id}")
        ...
        ScoutClientHandler.websocket_url = websocket_url
        ScoutClientHandler.session_id = session_id
        ScoutClientHandler.is_presigned = is_presigned
        ScoutClientHandler.agent_name = args.agent_name

        # Store config for regenerating URLs
        if args.runtime_arn:
            ScoutClientHandler.runtime_arn = args.runtime_arn
            ScoutClientHandler.region = args.region
            ScoutClientHandler.service = args.service
            ScoutClientHandler.expires = args.expires
            ScoutClientHandler.qualifier = args.qualifier
            ScoutClientHandler.voice_id = args.voice_id

        # Start web server
        server_address = ("", args.port)
        httpd = HTTPServer(server_address, ScoutClientHandler)
